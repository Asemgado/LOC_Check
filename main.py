...
import tempfile, os, uvicorn, uuid, json, requests,base64
...
# Initialize Firebase Admin SDK (will need proper service account key)
# For now, we'll use a simple approach with Firebase Storage
...
async def inspect_sealing(
    ...
    if not image:
        response = await text_response(prompt, SEALING_PROMPT)  # type: ignore
    ...
    await image.seek(0)
    response = await image_response(image, SEALING_PROMPT, prompt)  # type: ignore
...
async def inspect_vault_flooding(
    ...
    if not image:
        response = await text_response(prompt, VAULT_FLOODING_PROMPT)  # type: ignore
    ...
    # Reset file pointer for analysis
    await image.seek(0)
    response = await image_response(image, VAULT_FLOODING_PROMPT, prompt)  # type: ignore
...
async def inspect_duct_bend(
    ...
    if not image:
        response = await text_response(prompt, DUCT_BEND_PROMPT)  # type: ignore
    ...
    # Reset file pointer for analysis
    await image.seek(0)
    response = await image_response(image, DUCT_BEND_PROMPT, prompt)  # type: ignore
